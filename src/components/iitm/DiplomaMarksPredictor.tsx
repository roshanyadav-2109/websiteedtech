...
import { calculateDiplomaGrade } from "./utils/gradeCalculations";

interface DiplomaMarksPredictorProps {
  branch: string;
  level: string;
}
...
function calcRequiredF(subjectKey: string, inputs: Record<string, number>, targetT: number): number | null {
  ...
  for (let F = 0; F <= 100; F++) {
    const score = calculateDiplomaGrade(subjectKey, { ...inputs, F });
...
function checkEligibility(subjectKey: string, inputs: Record<string, number>): string | null {
  const GAA_val = inputs.GAA ?? inputs.GAA1 ?? inputs.GA ?? 0;
  
  if (GAA_val < 40) {
    return "Eligibility: Assignment average must be at least 40/100 to appear for end term.";
  }
  
  const quizSubjects = ["math_electronics2", "signals_systems", "analog_electronic_systems", "digital_signal_processing", "sensors_applications", "digital_system_design", "control_engineering"];
  if (quizSubjects.includes(subjectKey)) {
    if ((inputs.Qz1 ?? 0) <= 0 && (inputs.Qz2 ?? 0) <= 0) {
        return "Eligibility: At least one quiz (Qz1 or Qz2) must be attended (>0).";
    }
  }

  if (subjectKey === "python_programming_es") {
    if ((inputs.PE1 ?? 0) < 40 && (inputs.PE2 ?? 0) < 40) {
      return "Note: To be eligible for a grade, at least one of PE1 or PE2 must be ≥ 40.";
    }
  }
...
export default function DiplomaMarksPredictor({ branch, level }: DiplomaMarksPredictorProps) {
  const [inputs, setInputs] = useState<Record<string, string>>({});
  
  const getSubjectsKey = () => {
    if (branch === "electronic-systems" && level === "diploma") {
      return "diploma-electronic-systems";
    }
    return "diploma";
  };
  ...
  const GAA_val = numericInputs.GAA ?? numericInputs.GAA1 ?? numericInputs.GA ?? 0;

  const currentScore = useMemo(() => subjectObj ? calculateDiplomaGrade(subjectKey, { ...numericInputs, F: 0 }) : 0, [subjectKey, numericInputs, subjectObj]);
  ...
  React.useEffect(() => {
      if (subjects.length > 0 && !subjects.find(s => s.key === subjectKey)) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Diploma Subject Final Exam Marks Predictor</CardTitle>
        <div className="text-sm text-gray-600">
          {branch === "electronic-systems" ? "BS Electronic Systems" : "BS Data Science"} - Diploma Level
        </div>
      </CardHeader>
      <CardContent>
